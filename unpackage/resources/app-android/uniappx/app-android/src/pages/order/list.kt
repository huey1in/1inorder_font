...
open class GenPagesOrderList : BasePage {
    ...
    override fun `$render`(): Any? {
        ...
        return _cE("view", _uM("class" to "container"), _uA(
            _cE("view", _uM("class" to "status-tabs"), _uA(
            ...
            _cE("scroll-view", _uM("class" to "order-list", "scroll-y" to "", "onScrolltolower" to _ctx.loadMore), _uA(
                _cE(Fragment, null, RenderHelpers.renderList(_ctx.orders, fun(order, __key, __index, _cached): Any {
                    return _cE("view", _uM("class" to "order-card", "key" to order.id, "onClick" to fun(){
                        ...
                        _cE("view", _uM("class" to "order-header"), _uA(
                            ...
                            _cE("text", _uM("class" to _nC(_uA(
                                "order-status",
                                "status-" + order.status
                            ))), _tD(_ctx.getStatusText(order.status)), 3)
                        )),
                        ...
                        _cE("view", _uM("class" to "order-footer"), _uA(
                            ...
                            _cE("view", _uM("class" to "order-actions"), _uA(
                                ...
                                if (order.status === "completed") {
                                    _cE("view", _uM("key" to 2, "class" to "action-btn primary", "onClick" to withModifiers(fun(){
                                        _ctx.reorder(order)
                                    }, _uA(
                                        "stop"
                                    ))), _uA(
                                        _cE("text", _uM("class" to "btn-text-primary"), "再来一单")
                                    ), 8, _uA(
                                        "onClick"
                                    ))
                                } else {
                ...
                if (isTrue(_ctx.orders.length === 0 && !_ctx.loading)) {
    ...
    override fun data(): Map<String, Any?> {
        ...
        ), "currentStatus" to "", "orders" to _uA<Order>(), "loading" to false, "noMore" to false, "page" to 1)
    }
    ...
    open fun gen_getStatusText_fn(status: String): String {
        val map = Record(pending = "待付款", paid = "制作中", completed = "已完成", cancelled = "已取消")
