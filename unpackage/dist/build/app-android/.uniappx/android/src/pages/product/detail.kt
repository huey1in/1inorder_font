...
open class GenPagesProductDetail : BasePage {
        ...
        return _cE("view", _uM("class" to "container"), _uA(
            ...
            _cE("view", _uM("class" to "product-info"), _uA(
                ...
                _cE("view", _uM("class" to "price-row"), _uA(
                    ...
                    ,
                    _cE("text", _uM("class" to "sales"), "已售 " + _tD(_ctx.product.sales_count || 0), 1)
                )),
                _cE("view", _uM("class" to "meta-row"), _uA(
                    if (isTrue(_ctx.product.category_name)) {
                        _cE("text", _uM("key" to 0, "class" to "meta-text"), "分类: " + _tD(_ctx.product.category_name), 1)
                    } else {
                        _cC("v-if", true)
                    }
                    ,
                    _cE("text", _uM("class" to "meta-text"), "库存: " + _tD(_ctx.product.stock_quantity || 0), 1)
                ))
            )),
            _cE("view", _uM("class" to "detail-section"), _uA(
                _cE("text", _uM("class" to "section-title"), "商品详情"),
                _cE("view", _uM("class" to "detail-content"), _uA(
                    _cE("text", _uM("class" to "detail-text"), _tD(_ctx.product.detail || _ctx.product.description || "暂无详细介绍"), 1)
                ))
            )),
    ...
    override fun data(): Map<String, Any?> {
        return _uM("productId" to 0, "defaultFoodImage" to DEFAULT_FOOD_IMAGE, "product" to object : UTSJSONObject() {
            ...
            var sales_count: Number = 0
            var stock_quantity: Number = 0
            var category_name = ""
        }, "cartCount" to 0)
    }
